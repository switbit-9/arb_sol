...
// Note: This is a workaround since dlmm::program::ID is private
// The ID matches the address in the IDL file
...
pub fn derive_oracle_pda(lb_pair: Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[ORACLE, lb_pair.as_ref()], &get_dlmm_id())
}
...
pub fn derive_reserve_pda(token_mint: Pubkey, lb_pair: Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[lb_pair.as_ref(), token_mint.as_ref()], &get_dlmm_id())
}
...
pub fn derive_preset_parameter_pda(bin_step: u16) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[PRESET_PARAMETER, &bin_step.to_le_bytes()], &get_dlmm_id())
}
...
pub fn derive_preset_parameter_pda_v2(index: u16) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[PRESET_PARAMETER2, &index.to_le_bytes()], &get_dlmm_id())
}

pub fn derive_token_badge_pda(mint: Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[TOKEN_BADGE, mint.as_ref()], &get_dlmm_id())
}
